...
export function formatOddsToCents(int: bigint | undefined): string {
  if (!int) return "--¢";
  // Divide by 2^64 to get decimal, then convert to cents
  return `${Math.round((Number(int) / Math.pow(2, 64)) * 100)}¢`;
}
